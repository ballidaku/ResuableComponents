...
import ballidaku.resuablecomponents.myInstagram.adapter.RelationShipAdapter;
import ballidaku.resuablecomponents.myInstagram.util.JSONParser;
...
public class Relationship extends Activity
{
    ...
    private void setImageGridAdapter() {
        lvRelationShipAllUser.setAdapter(new RelationShipAdapter(context,
                usersInfo));
        Log.d(TAG, "setImageGridAdapter() called with: " + usersInfo.size() + "");

        lvRelationShipAllUser.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent(Relationship.this, DashBoardActivty.class);

                intent.putExtra(Intent.EXTRA_TEXT, usersInfo.size());
